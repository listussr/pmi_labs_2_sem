...
enum State // состояния автомата
{
	...
	Id_5, Minus, Plus, More, Fewer, More_or_Equal, Fewer_or_Equal, Equal, A, B,	C, d,
	More_1, More_2, More_3, More_4, ERROR, Final, Last_digit
};
...
void filling_automat_table(State(&table)[24][36]) { // заполнение автомата
	// для S
...
lex_type Get_Lex_Type(State st) { // смотрим тип лексемы по выходному состоянию
	...
	else if (st == State::A || st == State::B || st == State::C || st == State::d || st == State::More_1
...
void Lex_analys(vector<Lex>& res, char* mass, State table[24][36], int mass_sz) { // основная функция
	Lex lex{};
	...
	while (counter != mass_sz) {
		...
		cur = table[get_signal(marker)][cur]; // переход по матрице состояний

		if (cur == State::Final) { // если есть разделение пробелом или чем-то ещё
		...
		// здесь все случаи, в которых разделетили - арифметические операции и операции сравнения
...
void writln(ofstream& out, vector<Lex>& vec) { // вывод в файл
